...
gen_ifconfig() {
    cat <<EOF
$(awk -F "/" '{print "ifconfig eth0 inet6 add " $3 "/48"}' ${WORKDATA})
EOF
}
...
yum install -y gcc make net-tools curl wget >/dev/null
...
echo "Internal IP = ${IP4}. External subnet for IPv6 = ${IP6}::/48"
...
while :; do
  ...
  if ((FIRST_PORT >= 10000 && FIRST_PORT <= 60000)); then
    ...
    read -p "Enter NUMBER of proxies to create: " NUMBER_PROXY
