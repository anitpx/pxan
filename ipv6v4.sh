...
then
   tee -a /etc/sysconfig/network-scripts/ifcfg-eth0 <<-EOF
	...
	IPV6ADDR=2001:0df4:2540:0400::$IPD:0000/56
	IPV6_DEFAULTGW=2001:0df4:2540:0400::1
	EOF
elif [ $IPC == 17 ]
then
   tee -a /etc/sysconfig/network-scripts/ifcfg-eth0 <<-EOF
	...
	IPV6_FAILURE_FATAL=no
	IPV6_ADDR_GEN_MODE=stable-privacy
	IPV6ADDR=2001:0df4:2540:0500::$IPD:0000/56
	IPV6_DEFAULTGW=2001:0df4:2540:0500::1
	EOF
elif [ $IPC == 141 ]
then
   tee -a /etc/sysconfig/network-scripts/ifcfg-eth0 <<-EOF
...
then
   tee -a /etc/sysconfig/network-scripts/ifcfg-eth0 <<-EOF
	...
	IPV6_FAILURE_FATAL=no
	IPV6_ADDR_GEN_MODE=stable-privacy
	IPV6ADDR=2001:0df4:2540:1000::$IPD:0000/56
	IPV6_DEFAULTGW=2001:0df4:2540:1000::1
	EOF
elif [ $IPC == 13 ]
then
   tee -a /etc/sysconfig/network-scripts/ifcfg-eth0 <<-EOF
...
then
   tee -a /etc/sysconfig/network-scripts/ifcfg-eth0 <<-EOF
	...
	IPV6_FAILURE_FATAL=no
	IPV6_ADDR_GEN_MODE=stable-privacy
	IPV6ADDR=2001:0df4:2540::$IPD:0000/56
	IPV6_DEFAULTGW=2001:0df4:2540::1
	EOF
elif [ $IPC == 4 ]
then
   tee -a /etc/sysconfig/network-scripts/ifcfg-eth0 <<-EOF
	...
	IPV6_FAILURE_FATAL=no
	IPV6_ADDR_GEN_MODE=stable-privacy
	IPV6ADDR=2001:0df4:2540:0200::$IPD:0000/56
	IPV6_DEFAULTGW=2001:0df4:2540:0200::1
	EOF
elif [ $IPC == 5 ]
then
   tee -a /etc/sysconfig/network-scripts/ifcfg-eth0 <<-EOF
...
then
   tee -a /etc/sysconfig/network-scripts/ifcfg-eth0 <<-EOF
	...
	IPV6_FAILURE_FATAL=no
	IPV6_ADDR_GEN_MODE=stable-privacy
	IPV6ADDR=2001:0df4:2540:0900::$IPD:0000/56
	IPV6_DEFAULTGW=2001:0df4:2540:0900::1
	EOF
elif [ $IPC == 89 ]
then
   tee -a /etc/sysconfig/network-scripts/ifcfg-eth0 <<-EOF
	...
	IPV6_FAILURE_FATAL=no
	IPV6_ADDR_GEN_MODE=stable-privacy
	IPV6ADDR=2001:0df4:2540:0700::$IPD:0000/56
	IPV6_DEFAULTGW=2001:0df4:2540:0700::1
	EOF
elif [ $IPC == 90 ]
then
   tee -a /etc/sysconfig/network-scripts/ifcfg-eth0 <<-EOF
